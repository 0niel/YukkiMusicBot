...
class Call(PyTgCalls):
    ...
    async def join_assistant(self, original_chat_id, chat_id):
        ...
        try:
            ...
            if get.status in ["banned", "kicked"]:
                raise AssistantErr(_["call_2"].format(
            ...
            if chat.username:
                ...
                try:
                    try:
                        ...
                        raise AssistantErr(_["call_4"]) from e
                    except Exception as e:
    ...
    async def join_call(self, chat_id: int, original_chat_id: int, link, video: Union[bool, str] = None):
        ...
        audio_stream_quality = await get_audio_bitrate(chat_id)
        video_stream_quality = await get_video_bitrate(chat_id)
        stream = AudioVideoPiped(link, audio_parameters=audio_stream_quality, video_parameters=video_stream_quality) if video else AudioPiped(
            link, audio_parameters=audio_stream_quality)
        ...
        if await is_autoend():
            ...
            if users == 1:
                ...
                ) + timedelta(minutes=AUTO_END_TIME)
    ...
    async def change_stream(self, client, chat_id):
        ...
        try:
            ...
            if "live_" in queued:
                ...
                stream = AudioVideoPiped(link, audio_parameters=audio_stream_quality, video_parameters=video_stream_quality) if str(
                    streamtype) == "video" else AudioPiped(link, audio_parameters=audio_stream_quality)
                ...
                run = await app.send_photo(original_chat_id, photo=img, caption=_["stream_1"].format(user, f"https://t.me/{app.username}?start=info_{videoid}"), reply_markup=InlineKeyboardMarkup(button))
                ...
                stream = AudioVideoPiped(file_path, audio_parameters=audio_stream_quality, video_parameters=video_stream_quality) if str(
                    streamtype) == "video" else AudioPiped(file_path, audio_parameters=audio_stream_quality)
                ...
                stream = AudioVideoPiped(videoid, audio_parameters=audio_stream_quality, video_parameters=video_stream_quality) if str(
                    streamtype) == "video" else AudioPiped(videoid, audio_parameters=audio_stream_quality)
                ...
                try:
                    ...
                    return await app.send_message(original_chat_id, text=_["call_9"])
                button = telegram_markup(_, chat_id)
                run = await app.send_photo(original_chat_id, photo=config.STREAM_IMG_URL, caption=_["stream_2"].format(user), reply_markup=InlineKeyboardMarkup(button))

                db[chat_id][0]["mystic"] = run
                db[chat_id][0]["markup"] = "tg"
            else:
                stream = AudioVideoPiped(queued, audio_parameters=audio_stream_quality, video_parameters=video_stream_quality) if str(
                    streamtype) == "video" else AudioPiped(queued, audio_parameters=audio_stream_quality)
                ...
                if videoid == "telegram":
                    button = telegram_markup(_, chat_id)
                    run = await app.send_photo(original_chat_id, photo=config.TELEGRAM_AUDIO_URL if str(streamtype) == "audio" else config.TELEGRAM_VIDEO_URL, caption=_["stream_3"].format(title, check[0]["dur"], user), reply_markup=InlineKeyboardMarkup(button))

                    db[chat_id][0]["mystic"] = run
                    db[chat_id][0]["markup"] = "tg"
                elif videoid == "soundcloud":
                    button = telegram_markup(_, chat_id)
                    run = await app.send_photo(original_chat_id, photo=config.SOUNCLOUD_IMG_URL, caption=_["stream_3"].format(title, check[0]["dur"], user), reply_markup=InlineKeyboardMarkup(button))
    ...
    async def decorators(self):
        ...
        async def participants_change_handler(client, update: Update):
            ...
            if not users:
                ...
                if got == 1:
                    autoend[chat_id] = (
                        datetime.now() + timedelta(minutes=AUTO_END_TIME))
                    return
                autoend[chat_id] = {}
            else:
                final = users + \
